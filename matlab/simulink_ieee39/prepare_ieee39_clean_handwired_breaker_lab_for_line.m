function summary = prepare_ieee39_clean_handwired_breaker_lab_for_line(lineId, sourceWrapperPath, outputDir)
%PREPARE_IEEE39_CLEAN_HANDWIRED_BREAKER_LAB_FOR_LINE Copy one clean lab per line.
%
% This helper creates a local clean lab model for one manual breaker target.
% It does not insert breakers, reconnect Simscape physical ports, or modify
% the source wrapper. The generated .slx is local-only and must not be
% committed.

if nargin < 1 || isempty(lineId)
    lineId = "L03";
end
    ...
    if ~sourceFound
        ...
        if sourceContains
            ...
            if targetCreated
                ...
                note = "per-line clean breaker lab copied from source wrapper for " + lineId + "; no breaker was inserted";
            end
        end
    end
catch ME
...
writeJson(summary, fullfile(outputDir, "ieee39_clean_breaker_lab_per_line_prepare_summary.json"));
fprintf("Wrote IEEE39 per-line clean breaker lab prepare summary under: %s\n", outputDir);
end
