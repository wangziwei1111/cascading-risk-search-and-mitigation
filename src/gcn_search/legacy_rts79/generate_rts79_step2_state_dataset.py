...
def consolidate_step2_state_checkpoints(output_dir: str | Path, config: Step2StateDatasetConfig) -> dict:
    ...
    if not checkpoint_paths:
        raise RuntimeError(f"No Step2-State checkpoints found: {checkpoint_dir}")
    ...
    np.savez(
        ...
        x_gcn=x_gcn,
        x_gcn_raw=x_raw,
        physics_raw_features=x_raw if config.feature_mode == "physics" else np.zeros((0,), dtype=np.float32),
    ...
    return {"x_gcn": x_gcn, "x_gcn_raw": x_raw, "y_one_step": y_one_step, "y_reachable": y_reachable, "loss_mask": loss_mask}
...
def _generate_one_scenario_checkpoint(scenario_id: int, seed: int, config: Step2StateDatasetConfig) -> dict:
    ...
    for local_state_id, (active_sequence, state_case) in enumerate(state_specs, start=1):
        print(f"[step2] scenario={scenario_id}/{config.num_scenarios}, local_state={local_state_id}, active_sequence={_format_sequence(active_sequence)}")
        x_gcn = _make_state_x_gcn(state_case, config)
        y_one, y_reach, mask = _label_state(active_sequence, initial_config, state_case, config)
...
def _simulate_one_step_from_state(state_case: dict, candidate: str, config: Step2StateDatasetConfig) -> float:
    ...
    from rts79_cascade import _build_branch_table, balance_islands_by_load_shedding, redispatch_minimize_load_shed, solve_islanded_dcpf
...
def _simulate_reachable_from_initial_sequence(next_sequence: tuple[str, ...], initial_config: Rts79InitialConfig, config: Step2StateDatasetConfig) -> float:
    if len(next_sequence) >= config.random_failure_limit_r:
        result = simulate_cascade_path(next_sequence, config=initial_config, relay_threshold_beta=config.relay_threshold_beta, security_limit=config.security_limit)
    ...
    for line_idx in range(1, 39):
        ...
        if candidate in used:
            continue
        result = simulate_cascade_path(tuple(list(next_sequence) + [candidate]), config=initial_config, relay_threshold_beta=config.relay_threshold_beta, security_limit=config.security_limit)
