...
def main() -> int:
    ...
    spp001_builder_dir = ROOT / "results/gcn_search/ieee39_spp001_same_wrapper_bridge_builder_repair"
    spp001_builder_doc = ROOT / "docs/ieee39_spp001_same_wrapper_bridge_builder_repair.md"
    spp001_builder_summary = spp001_builder_dir / "spp001_bridge_builder_repair_summary.json"
    spp001_builder_report = spp001_builder_dir / "spp001_bridge_builder_repair_report.json"
    spp001_builder_manifest = spp001_builder_dir / "spp001_repaired_same_wrapper_manifest.json"
    spp001_builder_gate = spp001_builder_dir / "spp001_bridge_builder_rerun_gate.json"
    spp001_builder_no_leakage = spp001_builder_dir / "no_leakage_spp001_bridge_builder_repair_audit.json"
    spp001_builder_safety = spp001_builder_dir / "large_file_safety_spp001_bridge_builder_repair.json"
    spp001_builder_required = [
        spp001_builder_doc,
        spp001_builder_summary,
        spp001_builder_dir / "spp001_bridge_builder_repair_summary.md",
        spp001_builder_dir / "spp001_bridge_builder_repair_summary.csv",
        spp001_builder_report,
        spp001_builder_dir / "spp001_bridge_builder_repair_report.md",
        spp001_builder_manifest,
        spp001_builder_dir / "spp001_repaired_same_wrapper_manifest.md",
        spp001_builder_gate,
        spp001_builder_dir / "spp001_bridge_builder_rerun_gate.md",
        spp001_builder_no_leakage,
        spp001_builder_dir / "no_leakage_spp001_bridge_builder_repair_audit.md",
        spp001_builder_safety,
        spp001_builder_dir / "large_file_safety_spp001_bridge_builder_repair.md",
    ]
    existing_spp001_builder_required = [path for path in spp001_builder_required if path.exists()]
    if existing_spp001_builder_required:
        missing_spp001_builder_required = [path for path in spp001_builder_required if not path.exists()]
        if missing_spp001_builder_required:
            failures.append(
                "IEEE39 SPP001 same-wrapper bridge builder repair artifacts are partially present but incomplete:\n"
                + "\n".join(f"  - missing {path.relative_to(ROOT)}" for path in missing_spp001_builder_required)
            )
        try:
            summary = _read_json_path(spp001_builder_summary)
            report = _read_json_path(spp001_builder_report)
            manifest = _read_json_path(spp001_builder_manifest)
            gate = _read_json_path(spp001_builder_gate)
            no_leakage = _read_json_path(spp001_builder_no_leakage)
            safety = _read_json_path(spp001_builder_safety)
            for key, expected in [
                ...
                if summary.get(key) != expected:
                    failures.append(f"SPP001 bridge builder repair summary must set {key}={expected!r}.")
            if "approve rerun of SPP001 single-pair smoke using repaired same-wrapper bridge" not in str(summary.get("recommended_next_step")):
                failures.append("SPP001 builder repair must recommend a separate SPP001 smoke rerun approval.")
            if report.get("repair_report_scope") != "spp001_bridge_builder_repair_report":
                failures.append("SPP001 builder repair report has wrong scope.")
            if report.get("matlab_builder_invoked") is not True or report.get("matlab_returncode") != 0:
                failures.append("SPP001 builder repair report must show successful guarded MATLAB builder invocation.")
            if report.get("local_bridge_built") is not True or report.get("source_slx_modified") is not False:
                failures.append("SPP001 builder repair report must build only the local bridge and not modify source SLX.")
            if manifest.get("manifest_scope") != "spp001_repaired_same_wrapper_manifest":
                failures.append("SPP001 repaired same-wrapper manifest has wrong scope.")
            for key, expected in [
                ("same_wrapper_confirmed", True),
                ("l15_trip_command_found_in_bridge", True),
                ("l04_trip_command_found_in_bridge", True),
                ...
                if manifest.get(key) != expected:
                    failures.append(f"SPP001 repaired manifest must set {key}={expected!r}.")
            if gate.get("gate_scope") != "spp001_bridge_builder_rerun_gate":
                failures.append("SPP001 builder repair rerun gate has wrong scope.")
            for key, expected in [
                ("l15_ready", True),
                ("l04_ready", True),
                ("local_bridge_built", True),
                ...
                ("formal_label_export_allowed", False),
                ("gcn_training_allowed", False),
                ("can_request_spp001_smoke_rerun_approval", True),
            ]:
                if gate.get(key) != expected:
                    failures.append(f"SPP001 builder repair gate must set {key}={expected!r}.")
            for key, expected in [
                ("forbidden_features_detected_in_inputs", []),
                ("post_fault_dynamic_measurements_used_as_inputs", False),
                ("dynamic_outputs_used_only_as_future_labels_or_targets", True),
                ...
                if no_leakage.get(key) != expected:
                    failures.append(f"SPP001 builder repair no-leakage audit must set {key}={expected!r}.")
            for key in [
                "raw_trajectories_committed",
                "full_timeseries_committed",
                "mat_files_committed",
                ...
                if safety.get(key) is not False:
                    failures.append(f"SPP001 builder repair safety check must keep {key}=false.")
            if safety.get("safety_check_passed") is not True:
                failures.append("SPP001 builder repair large-file safety check must pass.")
            builder_text = "\n".join(
                [
                    _read_text("docs/ieee39_spp001_same_wrapper_bridge_builder_repair.md"),
                    _read_text("docs/gcn_pio_validation_log.md"),
                    _read_text("results/gcn_search/ieee39_spp001_same_wrapper_bridge_builder_repair/spp001_bridge_builder_repair_summary.md"),
                    _read_text("results/gcn_search/ieee39_spp001_same_wrapper_bridge_builder_repair/spp001_bridge_builder_rerun_gate.md"),
                ]
            ).lower()
            normalized_builder = " ".join(builder_text.replace("`", "").split())
            for required in [
                ...
                if required not in normalized_builder:
                    failures.append(f"SPP001 builder repair docs missing: {required}")
            for bad in [
                "gcn is useful",
                "gcn is useless",
                "spp001 smoke completed",
                ...
                if bad in normalized_builder:
                    failures.append(f"SPP001 builder repair docs contain overstatement: {bad}")
        except Exception as exc:
