...
def main():
    ...
    oracle_path = base / "eval" / "eval_do_nothing_agent_oracle_100.csv"
    if oracle_path.exists():
        _write_before_after(_read_csv(oracle_path), out / "table_ieee14_oracle_diagnostic_summary.csv")
...
def _write_explanation(path, recommended_scale):
    path.write_text(
        "# IEEE14 小系统实验表格说明\n\n"
        "表格基于 PYPOWER IEEE14 小系统、N-1/共同母线 N-2 初始故障采样，以及 scaled_from_base_flow 容量校准得到。"
        "before/after 评估使用同一批 scenario，因此可用于比较 do-nothing 与 PPO agent 在负回报、级联代数、断线数和切负荷等指标上的差异。\n\n"
        f"容量校准脚本推荐的 rate_a_scale 为 `{recommended_scale}`。该值用于构造小系统压力场景，不等同于论文原始系统参数。\n\n"
        "若 one-step oracle 优于 do-nothing 而 PPO agent 未改善，应表述为当前 PPO 训练未能利用可改善动作；"
        "one-step oracle 只能作为诊断上界。不要表述为精确复现原论文 Figure 7/8 的数值结果。\n",
        encoding="utf-8",
    )
