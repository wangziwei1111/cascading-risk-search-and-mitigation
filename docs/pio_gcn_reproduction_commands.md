...
Purpose: check simulator consistency, feature generation, mask/loss logic, measured-state handling, and rank-loss behavior.
...
Purpose: generate Step2-state training data with physics features.
...
Purpose: train the physics-feature GCN. Model input uses normalized features; physics loss uses raw physical features.
...
```powershell
& "C:\Users\24186\Documents\New project 7\.venv\Scripts\python.exe" src/gcn_search/legacy_rts79/run_pio_gcn_formal_experiment.py --output-dir results/gcn_search/pio_formal_preliminary_3seed --test-seed-start 20260722 --test-num-seeds 3 --top-k 20 50 100
```
...
Purpose: compare physics features, candidate mask, physics loss, paper GCN, LODF_yP, random, line order, and oracle.
...
## Rank-Loss Experiment

Purpose: test whether pairwise reachable ranking loss improves Top-K sorting.
...
Figures are generated by the formal experiment, ablation, and rank-loss scripts. Main output directories:
