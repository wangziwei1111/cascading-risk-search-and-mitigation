...
def test_oracle_bc_writes_requested_diagnostics(tmp_path):
    ...
    assert (tmp_path / "oracle_bc_dataset.csv").exists()
    assert (tmp_path / "oracle_bc_diagnostics.json").exists()
    assert {"num_train_scenarios", "num_bc_samples", "action_distribution", "mean_train_improvement", "mean_prob_best_action_after_bc", "mean_entropy_after_bc"} <= set(diagnostics)
