...
def test_spp001_same_wrapper_bridge_dry_run_writes_artifacts() -> None:
    subprocess.run(
        [
            sys.executable,
...
def test_spp001_same_wrapper_bridge_dry_run_artifacts_exist() -> None:
    expected = [
        "spp001_same_wrapper_bridge_dry_run_summary.json",
        "spp001_same_wrapper_bridge_dry_run_summary.md",
        "spp001_same_wrapper_bridge_dry_run_summary.csv",
        "spp001_bridge_component_plan.json",
        "spp001_bridge_component_plan.md",
        "spp001_same_wrapper_candidate_manifest.json",
        "spp001_same_wrapper_candidate_manifest.md",
        "spp001_bridge_local_execution_checklist.md",
        "spp001_bridge_readiness_gate.json",
        "spp001_bridge_readiness_gate.md",
        "no_leakage_spp001_bridge_dry_run_audit.json",
        "no_leakage_spp001_bridge_dry_run_audit.md",
        "large_file_safety_spp001_bridge_dry_run.json",
        "large_file_safety_spp001_bridge_dry_run.md",
    ]
...
def test_spp001_same_wrapper_bridge_dry_run_summary_and_gate() -> None:
    summary = _read_json(OUT_DIR / "spp001_same_wrapper_bridge_dry_run_summary.json")
    manifest = _read_json(OUT_DIR / "spp001_same_wrapper_candidate_manifest.json")
    gate = _read_json(OUT_DIR / "spp001_bridge_readiness_gate.json")
    plan = _read_json(OUT_DIR / "spp001_bridge_component_plan.json")
    ...
    assert summary["recommended_next_step"] == "approve local SPP001 same-wrapper bridge build/validation in a separate round; do not run smoke yet"

    assert manifest["manifest_scope"] == "spp001_same_wrapper_candidate_manifest"
    assert manifest["same_wrapper_confirmed_now"] is False
    assert manifest["same_wrapper_bridge_planned"] is True
    assert manifest["same_wrapper_bridge_built_this_round"] is False
    assert manifest["approved_for_execution_now"] is False
    assert manifest["requires_next_round_approval"] is True

    assert gate["gate_scope"] == "spp001_same_wrapper_bridge_readiness_gate"
    assert gate["l15_ready"] is True
    assert gate["l04_ready"] is True
    assert gate["same_wrapper_confirmed_now"] is False
    assert gate["local_bridge_build_required"] is True
    ...
    assert gate["can_request_local_bridge_build_approval"] is True
    assert gate["can_request_spp001_smoke_rerun_approval"] is False

    assert plan["plan_scope"] == "spp001_same_wrapper_bridge_component_plan"
    assert plan["local_lab_copy_only"] is True
    assert plan["source_slx_modified"] is False
    assert plan["simulink_run"] is False
    assert plan["formal_labels_exported"] is False


def test_spp001_same_wrapper_bridge_dry_run_no_leakage_and_safety() -> None:
    no_leakage = _read_json(OUT_DIR / "no_leakage_spp001_bridge_dry_run_audit.json")
    safety = _read_json(OUT_DIR / "large_file_safety_spp001_bridge_dry_run.json")
...
def test_spp001_same_wrapper_bridge_dry_run_docs_are_conservative() -> None:
    text = "\n".join(
        [
            _read_text(DOC),
            _read_text(ROOT / "docs/gcn_pio_validation_log.md"),
            _read_text(OUT_DIR / "spp001_same_wrapper_bridge_dry_run_summary.md"),
            _read_text(OUT_DIR / "spp001_bridge_local_execution_checklist.md"),
        ]
    ).lower()
...
def test_spp001_same_wrapper_bridge_dry_run_no_forbidden_large_artifacts_tracked() -> None:
