...
OUT_DIR = ROOT / "results/gcn_search/ieee39_spp001_single_pair_smoke_execution"
DOC = ROOT / "docs/ieee39_spp001_single_pair_smoke_execution.md"
...
def test_spp001_runner_writes_compact_smoke_artifacts() -> None:
    subprocess.run(
        [
            sys.executable,
...
def test_spp001_artifacts_exist() -> None:
    expected = [
        "spp001_smoke_execution_approval.json",
        "spp001_smoke_execution_approval.md",
        "spp001_smoke_execution_summary.json",
        "spp001_smoke_execution_summary.md",
        "spp001_smoke_execution_summary.csv",
        "spp001_smoke_execution_result.json",
        "spp001_smoke_execution_result.md",
        "spp001_smoke_execution_result.csv",
        "spp001_no_leakage_smoke_audit.json",
        "spp001_no_leakage_smoke_audit.md",
        "spp001_large_file_safety_check.json",
        "spp001_large_file_safety_check.md",
    ]
...
def test_spp001_summary_result_and_approval_fields() -> None:
    approval = _read_json(OUT_DIR / "spp001_smoke_execution_approval.json")
    summary = _read_json(OUT_DIR / "spp001_smoke_execution_summary.json")
    result = _read_json(OUT_DIR / "spp001_smoke_execution_result.json")

    assert approval["approval_scope"] == "spp001_single_pair_smoke_execution_approval"
...
def test_spp001_no_leakage_and_large_file_safety() -> None:
    no_leakage = _read_json(OUT_DIR / "spp001_no_leakage_smoke_audit.json")
    safety = _read_json(OUT_DIR / "spp001_large_file_safety_check.json")
...
def test_spp001_docs_are_conservative() -> None:
    text = "\n".join(
        [
            _read_text(DOC),
            _read_text(ROOT / "docs/gcn_pio_validation_log.md"),
            _read_text(OUT_DIR / "spp001_smoke_execution_summary.md"),
            _read_text(OUT_DIR / "spp001_smoke_execution_result.md"),
        ]
    ).lower()
...
def test_spp001_runner_rejects_unapproved_pair_and_batch_execute() -> None:
    ...
    assert "spp001" in (bad_pair.stderr + bad_pair.stdout).lower()
    ...
    assert "max-pairs 1" in (bad_count.stderr + bad_count.stdout).lower()


def test_spp001_no_forbidden_large_artifacts_tracked() -> None:
