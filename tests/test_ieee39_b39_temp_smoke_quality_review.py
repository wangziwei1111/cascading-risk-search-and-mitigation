...
QUALITY_JSON = BASE / "ieee39_b39_temp_smoke_quality_review.json"
QUALITY_MD = BASE / "ieee39_b39_temp_smoke_quality_review.md"
DOCS = [
...
def test_quality_review_artifacts_exist() -> None:
    for path in [QUALITY_JSON, QUALITY_MD, ROOT / "docs/ieee39_b39_temp_smoke_quality_review.md"]:
        assert path.exists()
        assert path.stat().st_size > 0


def test_quality_review_passes_for_candidate_export() -> None:
    payload = _payload()
    ...
    assert payload["min_voltage_near_zero"] is True
    assert payload["unstable_flag"] is True
    assert payload["quality_review_passed_for_candidate_export"] is True


def test_quality_review_numeric_fields_are_available() -> None:
    payload = _payload()
    for key in [
        "min_voltage_pu",
        "max_voltage_pu",
...
def test_quality_review_preserves_boundaries() -> None:
    payload = _payload()
    ...
    assert payload["v2_candidate_count_preserved"] is True
    assert payload["v2_candidate_count"] == 40


def test_quality_review_docs_do_not_overclaim() -> None:
    ...
    for forbidden in [
        "is a formal label",
        "formal label export completed",
        "labels were exported successfully",
...
def test_no_forbidden_quality_review_artifacts_are_tracked() -> None:
