...
def test_clean_breaker_lab_prepare_summary_schema() -> None:
    root = Path(__file__).resolve().parents[1]
    path = root / "results/gcn_search/ieee39_graphical_dynamic_model/handwired_breaker_validation/ieee39_clean_breaker_lab_prepare_summary.json"
    assert path.exists()
    ...
    assert payload["contains_existing_L02_HandwiredTimedBreaker"] is False
