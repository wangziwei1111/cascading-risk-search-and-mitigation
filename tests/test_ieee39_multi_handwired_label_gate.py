...
def test_multi_handwired_label_gate_counts_unique_lines_and_blocks_training() -> None:
    ...
    assert quality["num_training_ready_labels"] == 4
    assert quality["num_training_ready_handwired_line_trip_labels"] == 2
    assert quality["num_training_ready_handwired_line_trip_labels_by_line"] == {"L01": 1, "L02": 1}
    assert quality["num_unique_handwired_line_ids"] == 2
