...
def test_spp001_timeout_diagnosis_writes_artifacts() -> None:
    subprocess.run(
        [
            sys.executable,
...
def test_spp001_timeout_diagnosis_artifacts_exist() -> None:
    expected = [
        "spp001_timeout_diagnosis_summary.json",
        "spp001_timeout_diagnosis_summary.md",
        "spp001_timeout_diagnosis_summary.csv",
        "spp001_timeout_phase_plan.json",
        "spp001_timeout_phase_plan.md",
        "spp001_timeout_stdout_stderr_excerpt.json",
        "spp001_timeout_stdout_stderr_excerpt.md",
        "spp001_timeout_diagnostic_gate.json",
        "spp001_timeout_diagnostic_gate.md",
        "no_leakage_spp001_timeout_diagnosis_audit.json",
        "no_leakage_spp001_timeout_diagnosis_audit.md",
        "large_file_safety_spp001_timeout_diagnosis.json",
        "large_file_safety_spp001_timeout_diagnosis.md",
    ]
...
def test_spp001_timeout_diagnosis_summary_contract() -> None:
    summary = _read_json(OUT_DIR / "spp001_timeout_diagnosis_summary.json")
    gate = _read_json(OUT_DIR / "spp001_timeout_diagnostic_gate.json")
    ...
    assert summary["likely_timeout_stage"] in {
        "unknown_before_phase_markers_or_repeated_codegen_setup",
        "unknown_without_phase_markers",
        summary.get("last_seen_phase_if_available"),
    }
    assert summary["recommended_next_step"] in {
        "run diagnostic-only SPP001 phase-timing check before any full smoke rerun",
        "approve one SPP001 diagnostic retry with phase timing focused on that stage; do not export labels or train",
    }

    assert gate["gate_scope"] == "spp001_timeout_diagnostic_gate"
    ...
    assert gate["can_request_spp001_diagnostic_retry"] is True
    assert gate["can_request_spp001_full_smoke_rerun"] is False


def test_spp001_timeout_diagnosis_no_leakage_and_safety() -> None:
    no_leakage = _read_json(OUT_DIR / "no_leakage_spp001_timeout_diagnosis_audit.json")
    safety = _read_json(OUT_DIR / "large_file_safety_spp001_timeout_diagnosis.json")
...
def test_spp001_timeout_diagnosis_docs_are_conservative() -> None:
    text = "\n".join(
        [
            _read_text(DOC),
            _read_text(ROOT / "docs/gcn_pio_validation_log.md"),
            _read_text(OUT_DIR / "spp001_timeout_diagnosis_summary.md"),
            _read_text(OUT_DIR / "spp001_timeout_phase_plan.md"),
        ]
    ).lower()
...
def test_spp001_timeout_diagnosis_no_forbidden_large_artifacts_tracked() -> None:
